...
SOCCER_CN = {
    ...
    "Athletic Club": "畢爾包競技", "Celta Vigo": "塞爾塔", "Osasuna": "奧薩蘇納", "Bayern Munich": "拜仁慕尼黑",
    "Bayer Leverkusen": "勒沃庫森", "Borussia Dortmund": "多特蒙德", "Internazionale": "國際米蘭", "Inter Milan": "國際米蘭",
...
def generate_soccer_report_cached(date_str: str):
    ...
    for l_name, matches in all_matches.items():
        ...
        for m in matches:
            ...
            rows.append(f'<tr><td style="padding: 7px; border: 1px solid #cbd5e1; font-weight: bold;">{h_cn} vs {a_cn}</td><td style="padding: 6px; border: 1px solid #cbd5e1; text-align: center;">{int(h_elo)} vs {int(a_elo)}</td><td style="padding: 6px; border: 1px solid #cbd5e1; text-align: center; color: #e11d48; font-weight: bold;">{lh:.2f}:{la:.2f}</td><td style="padding: 6px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold;">{m["score"]}</td><td style="padding: 6px; border: 1px solid #cbd5e1; text-align: center; font-weight: 600;">{p_1x2}</td><td style="padding: 6px; border: 1px solid #cbd5e1; text-align: center; font-weight: 600;">{p_ou}</td></tr>')
            
        t_block = f'<div style="margin-bottom: 20px;"><div style="background: linear-gradient(90deg, #0f172a, #334155); color: #fff; padding: 8px 12px; border-radius: 6px 6px 0 0; font-size: 13px; font-weight: bold;">{l_name}</div><div style="overflow-x: auto; -webkit-overflow-scrolling: touch; border: 1px solid #cbd5e1; border-top: none; border-radius: 0 0 6px 6px;"><table style="width: 100%; min-width: 680px; border-collapse: collapse; font-size: 12px; background-color: #fff;"><thead><tr style="background-color: #f1f5f9;"><th style="padding: 8px; border: 1px solid #cbd5e1;">對戰組合</th><th style="padding: 8px; border: 1px solid #cbd5e1;">ClubElo</th><th style="padding: 8px; border: 1px solid #cbd5e1;">預估 xG</th><th style="padding: 8px; border: 1px solid #cbd5e1;">真實比分</th><th style="padding: 8px; border: 1px solid #cbd5e1;">獨贏推薦</th><th style="padding: 8px; border: 1px solid #cbd5e1;">大小 (2.5)</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></div>'
...
class AutomatedMLBQuantSystem:
    ...
    def run_daily_pipeline(self, date_str: str = "2026-08-27", market_line: float = 8.5):
        ...
        for _, row in df_games.iterrows():
            ...
            ml_style, spread_style, ou_style = "padding: 8px;", "padding: 8px;", "padding: 8px;"
            ...
            if is_final:
                ...
                ml_style += "background-color: #c6efce; color: #006100; font-weight: bold;" if model_ml_pick_name == actual_ml_winner_name else "background-color: #ffc7ce; color: #9c0006;"
                    
                if spread_target == "HOME_COVER":
                    spread_style += "background-color: #c6efce; color: #006100; font-weight: bold;" if actual_diff > 1 else "background-color: #fff2cc; color: #8c6b00;" if actual_diff == 1 else "background-color: #ffc7ce; color: #9c0006;"
                elif spread_target == "AWAY_COVER":
                    spread_style += "background-color: #c6efce; color: #006100; font-weight: bold;" if actual_diff <= 0 else "background-color: #fff2cc; color: #8c6b00;" if actual_diff == 1 else "background-color: #ffc7ce; color: #9c0006;"
                ...
                if model_ou_pick == actual_ou:
                    ou_style += "background-color: #c6efce; color: #006100; font-weight: bold;"
                elif actual_ou == "走盤":
                    ou_style += "background-color: #fff2cc; color: #8c6b00;"
                else:
                    ou_style += "background-color: #ffc7ce; color: #9c0006;"
            else:
                actual_score_str = "未完賽"

            sp_text_away = f"{row['away_sp']}<br><span style='color: #64748b; font-size:11px;'>({away_data['hand']} | FIP {away_data['sp_advanced']:.2f})</span>" if row['away_sp'] != "TBD" else "TBD"
            sp_text_home = f"{row['home_sp']}<br><span style='color: #64748b; font-size:11px;'>({home_data['hand']} | FIP {home_data['sp_advanced']:.2f})</span>" if row['home_sp'] != "TBD" else "TBD"

            bp_text_away = f"<span style='font-size:10.5px;'>{away_data['fatigue_desc']}</span>"
            bp_text_home = f"<span style='font-size:10.5px;'>{home_data['fatigue_desc']}</span>"

            rows_html.append(f"""
                <tr>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold; white-space: nowrap;">{away_cn} @ {home_cn}</td>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; font-size: 11.5px; text-align: center; white-space: nowrap;">{sp_text_away}<br>vs<br>{sp_text_home}</td>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; text-align: center; white-space: nowrap;">{bp_text_away}<br>vs<br>{bp_text_home}</td>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; text-align: center; white-space: nowrap;">{umpire_display}</td>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; text-align: center; font-size: 11px; color: #0284c7; white-space: nowrap;">{weather_desc}</td>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; text-align: center; color: #e11d48; font-weight: bold; font-size: 12.5px; white-space: nowrap;">{lambda_away:.2f} : {lambda_home:.2f}</td>
                    <td style="padding: 8px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; white-space: nowrap;">{actual_score_str}</td>
                    <td style="{ml_style}; border: 1px solid #cbd5e1; text-align: center; white-space: nowrap;">{ml_text}</td>
                    <td style="{spread_style}; border: 1px solid #cbd5e1; text-align: center; white-space: nowrap;">{spread_pick}</td>
                    <td style="{ou_style}; border: 1px solid #cbd5e1; text-align: center; white-space: nowrap;">{ou_text}</td>
                </tr>
            """)
        ...
        <div style="margin-bottom: 20px; font-family: -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, sans-serif;">
            ...
            <div style="overflow-x: auto; -webkit-overflow-scrolling: touch; border: 1px solid #cbd5e1; border-top: none; border-radius: 0 0 6px 6px;">
                <table style="width: 100%; min-width: 900px; border-collapse: collapse; font-size: 12px; background-color: #ffffff;">
                    <thead>
                        <tr style="background-color: #f1f5f9; text-align: center; color: #1e293b; font-weight: bold;">
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">對戰組合</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">先發 & 慣用手</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">昨日牛棚狀態</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">本壘板主審</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">天氣與風向</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">預估分(客:主)</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">真實比分</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">獨贏推薦</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">讓分推薦 (±1)</th>
                            <th style="padding: 10px 8px; border: 1px solid #cbd5e1;">大小推薦 ({market_line})</th>
                        </tr>
                    </thead>
                    <tbody>
                        {''.join(rows_html)}
